...
export default function ProductosPage() {
  ...
  const openNew = (cat?: CategoriaProducto) => {
    setEditingId(null);
    const initial = { ...emptyProducto(), categoria: cat || ('bordado' as CategoriaProducto) };
    setForm(initial);
  ...
  const toggleActivo = (p: Producto) => {
    updateProducto({ ...p, activo: !p.activo });
    reload();
  };
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wide transition-colors border ${filterCat === 'all' ? 'bg-[#0a0a0a] text-white border-[#0a0a0a]' : 'bg-white text-neutral-400 border-neutral-200 hover:border-neutral-400'}`}
        >
        ...
        {categorias.map((c) => {
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wide transition-colors border ${filterCat === c.value ? 'bg-[#0a0a0a] text-white border-[#0a0a0a]' : 'bg-white text-neutral-400 border-neutral-200 hover:border-neutral-400'}`}
            >
              {c.label} ({count})
            </button>
          );
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          <button
            onClick={() => openNew(filterCat !== 'all' ? (filterCat as CategoriaProducto) : undefined)}
        ...
        <div className="space-y-6">
          {categorias
            .filter((c) => filterCat === 'all' || filterCat === c.value)
            .map((cat) => {
                ...
                <div key={cat.value}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-[#c72a09] uppercase tracking-wide hover:underline"
                    >
                      + Agregar
                    </button>
                  </div>
                  <div className="bg-white rounded-2xl border border-neutral-100">
                    {items.map((p, idx) => (
                      <div
                        key={p.id}
                        className={`flex items-center justify-between px-5 py-4 ${idx > 0 ? 'border-t border-neutral-50' : ''} ${!p.activo ? 'opacity-40' : ''} hover:bg-neutral-50/50 transition-colors`}
                      >
                        <div className="flex items-center gap-3">
                          <button
                            onClick={() => toggleActivo(p)}
                            className={`w-8 h-5 rounded-full transition-colors relative shrink-0 ${p.activo ? 'bg-[#c72a09]' : 'bg-neutral-200'}`}
                          >
                            <span
                              className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform shadow ${p.activo ? 'left-3.5' : 'left-0.5'}`}
                            />
                          </button>
                          <div>
                            <p className="text-sm font-semibold text-[#0a0a0a]">{p.nombre}</p>
                            <span
                              className={`text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded ${getCatInfo(p.categoria).color}`}
                            >
                              {getCatInfo(p.categoria).label}
                            </span>
                          </div>
                        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Playera Polo, Bordado Grande..."
