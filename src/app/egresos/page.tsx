    ...
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Editar Egreso' : 'Nuevo Egreso'}>
        <div className="space-y-4">
          ...
          <div><label className={labelClass}>Proveedor</label><select value={form.proveedorId} onChange={(e) => { if (e.target.value === '__new__') { window.open('/studio24/proveedores', '_blank'); return; } setForm({ ...form, proveedorId: e.target.value }); }} className={inputClass}><option value="">Sin proveedor</option><option value="__new__">+ Nuevo proveedor</option>{proveedores.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select></div>
          <div className="grid grid-cols-2 gap-4">
      ...
      <Modal open={recModalOpen} onClose={() => setRecModalOpen(false)} title={editingRecId ? 'Editar Recurrente' : 'Nuevo Recurrente'}>
        <div className="space-y-4">
          ...
          <div><label className={labelClass}>Proveedor</label><select value={recForm.proveedorId} onChange={(e) => { if (e.target.value === '__new__') { window.open('/studio24/proveedores', '_blank'); return; } setRecForm({ ...recForm, proveedorId: e.target.value }); }} className={inputClass}><option value="">Sin proveedor</option><option value="__new__">+ Nuevo proveedor</option>{proveedores.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select></div>
          <div className="grid grid-cols-2 gap-4">
