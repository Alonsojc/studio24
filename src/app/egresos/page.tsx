              ...
              {recurrentes.map((r) => (
                <div key={r.id} className={`flex items-center justify-between py-3 px-4 rounded-xl transition-all ${r.activo ? 'bg-white/[0.05]' : 'bg-white/[0.02] opacity-40'}`}>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => openEditRec(r)} className="text-white/30 hover:text-white text-xs">Editar</button>
                    <button onClick={() => handleDeleteRec(r.id)} className="text-white/20 hover:text-red-400 text-xs">Eliminar</button>
                  </div>
                </div>
              ))}
            </div>
