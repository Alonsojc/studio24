...
export default function ProveedoresPage() {
  ...
  const filtered = proveedores.filter(
    (p) => p.nombre.toLowerCase().includes(search.toLowerCase()) || p.tipo.toLowerCase().includes(search.toLowerCase()),
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="bg-white rounded-2xl border border-neutral-100 p-6 hover:border-neutral-300 transition-all group relative"
              >
                ...
                {p.tipo && (
                  <span className="inline-block mt-1.5 px-2.5 py-0.5 rounded-lg text-[10px] font-bold tracking-wide bg-neutral-100 text-neutral-500 uppercase">
                    {p.tipo}
                  </span>
                )}
