    ...
    <div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Editar Ingreso' : 'Nuevo Ingreso'}>
        <div className="space-y-4">
          ...
          <div><label className={labelClass}>Cliente</label><select value={form.clienteId} onChange={(e) => { if (e.target.value === '__new__') { window.open('/studio24/clientes', '_blank'); return; } setForm({ ...form, clienteId: e.target.value }); }} className={inputClass}><option value="">Sin cliente</option><option value="__new__">+ Nuevo cliente</option>{clientes.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
          <div className="grid grid-cols-2 gap-4">
