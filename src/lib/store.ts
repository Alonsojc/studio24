...
/**
 * One-time cleanup: remove duplicate egresos (same fecha + descripcion + monto),
 * and remove duplicate ingresos/egresos with the same uuidCFDI.
 * Keeps the oldest record (by createdAt) in each duplicate group.
 * Returns the number of duplicates removed.
 */
const DEDUP_KEY = 'bordados_dedup_done_v1';

export function limpiarDuplicados(): number {
  if (typeof window === 'undefined') return 0;
  if (localStorage.getItem(DEDUP_KEY)) return 0;

  let removed = 0;

  // --- Deduplicate egresos ---
  const egresos = getEgresos();
  const egresosSeen = new Map<string, Egreso>();
  const egresosClean: Egreso[] = [];
  for (const e of egresos) {
    // Key by fecha + descripcion + monto for recurring duplicates
    const key = `${e.fecha}|${e.descripcion}|${e.monto}`;
    const existing = egresosSeen.get(key);
    if (existing) {
      // Keep the older one
      removed++;
    } else {
      egresosSeen.set(key, e);
      egresosClean.push(e);
    }
  }
  if (removed > 0) {
    setItems(KEYS.egresos, egresosClean);
  }

  // --- Deduplicate ingresos by uuidCFDI ---
  const ingresos = getIngresos();
  const uuidSeenIng = new Set<string>();
  const ingresosClean: Ingreso[] = [];
  for (const i of ingresos) {
    if (i.uuidCFDI && uuidSeenIng.has(i.uuidCFDI)) {
      removed++;
    } else {
      if (i.uuidCFDI) uuidSeenIng.add(i.uuidCFDI);
      ingresosClean.push(i);
    }
  }
  if (ingresosClean.length < ingresos.length) {
    setItems(KEYS.ingresos, ingresosClean);
  }

  // --- Deduplicate egresos by uuidCFDI (second pass on clean list) ---
  const uuidSeenEg = new Set<string>();
  const egresosClean2: Egreso[] = [];
  for (const e of egresosClean) {
    if (e.uuidCFDI && uuidSeenEg.has(e.uuidCFDI)) {
      removed++;
    } else {
      if (e.uuidCFDI) uuidSeenEg.add(e.uuidCFDI);
      egresosClean2.push(e);
    }
  }
  if (egresosClean2.length < egresosClean.length) {
    setItems(KEYS.egresos, egresosClean2);
  }

  // Mark cleanup as done so it never runs again
  safeSetItem(DEDUP_KEY, '1');
  return removed;
}
