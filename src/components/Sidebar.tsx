...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-[260px] bg-[#0a0a0a] text-white flex flex-col z-50">
      {/* Logo */}
      <div className="px-7 pt-8 pb-6">
        <div className="flex items-baseline gap-0">
          <span className="text-[28px] font-black tracking-[-0.06em] text-white uppercase">
            STUDIO
          </span>
          <span className="text-[28px] font-black tracking-[-0.06em] text-[#c72a09]">
            24
          </span>
        </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-lg text-[11px] font-semibold tracking-[0.08em] transition-all mb-1 ${
                isActive
      ...
      <div className="px-7 py-5 border-t border-white/[0.06]">
        <p className="text-[10px] tracking-[0.15em] text-neutral-600 uppercase">
          Bordados &amp; Servicios
        </p>
      </div>
    </aside>
