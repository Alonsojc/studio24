...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    getClientes().filter((c) => c.nombre.toLowerCase().includes(q) || c.email.toLowerCase().includes(q) || c.telefono.includes(q)).slice(0, 5).forEach((c) =>
      r.push({ type: 'Cliente', label: c.nombre, sub: c.telefono || c.email, href: '/clientes' })
    );

    getPedidos().filter((p) => p.descripcion.toLowerCase().includes(q)).slice(0, 5).forEach((p) =>
      r.push({ type: 'Pedido', label: p.descripcion, sub: formatCurrency(p.montoTotal), href: '/pedidos' })
    );

    getIngresos().filter((i) => i.descripcion.toLowerCase().includes(q)).slice(0, 3).forEach((i) =>
      r.push({ type: 'Ingreso', label: i.descripcion, sub: formatCurrency(i.montoTotal), href: '/ingresos' })
    );

    getEgresos().filter((e) => e.descripcion.toLowerCase().includes(q)).slice(0, 3).forEach((e) =>
      r.push({ type: 'Egreso', label: e.descripcion, sub: formatCurrency(e.montoTotal), href: '/egresos' })
    );

    getProveedores().filter((p) => p.nombre.toLowerCase().includes(q)).slice(0, 3).forEach((p) =>
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} className="w-full flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/[0.04] text-neutral-500 hover:text-white hover:bg-white/[0.08] transition-all text-xs">
    ...
    <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh]">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-100">
          <svg className="w-5 h-5 text-neutral-400 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg>
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          />
          <button onClick={() => setOpen(false)} className="text-[10px] text-neutral-400 bg-neutral-100 px-2 py-1 rounded-lg font-bold">ESC</button>
        </div>
        {results.length > 0 && (
          <div className="max-h-80 overflow-y-auto p-2">
            {results.map((r, idx) => (
              <button key={idx} onClick={() => go(r.href)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-neutral-50 text-left transition-colors">
                <span className={`px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wide shrink-0 ${typeColor[r.type] || 'bg-neutral-100 text-neutral-500'}`}>{r.type}</span>
